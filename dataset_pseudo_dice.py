...
def _random_patch_bounds(h, w, ph, pw):
    y0 = random.randint(0, h - ph)
    x0 = random.randint(0, w - pw)
    return y0, x0


def _apply_cutpaste_tensor(img_01, min_patch_ratio, max_patch_ratio):
    """CutPaste on tensor [3,H,W] in [0,1]. Returns modified img and binary mask [1,H,W]."""
    _, h, w = img_01.shape
    ph = max(8, int(h * random.uniform(min_patch_ratio, max_patch_ratio)))
    pw = max(8, int(w * random.uniform(min_patch_ratio, max_patch_ratio)))
    ph, pw = min(ph, h), min(pw, w)
    ys, xs = _random_patch_bounds(h, w, ph, pw)
    patch = img_01[:, ys:ys + ph, xs:xs + pw].clone()
    jitter = torch.rand(3, 1, 1, dtype=patch.dtype) * 0.4 + 0.8
    bias = (torch.rand(3, 1, 1, dtype=patch.dtype) - 0.5) * 0.15
    patch = torch.clamp(patch * jitter + bias, 0., 1.)
    yd, xd = _random_patch_bounds(h, w, ph, pw)
    out = img_01.clone()
    out[:, yd:yd + ph, xd:xd + pw] = patch
    mask = torch.zeros((1, h, w), dtype=torch.float32)
    mask[:, yd:yd + ph, xd:xd + pw] = 1.0
    return out, mask


def _apply_gaussian_noise_patch_tensor(img_01, min_patch_ratio, max_patch_ratio):
    """Random rectangular Gaussian noise patch. Returns modified img and mask [1,H,W]."""
    _, h, w = img_01.shape
    ph = max(8, int(h * random.uniform(min_patch_ratio, max_patch_ratio)))
    pw = max(8, int(w * random.uniform(min_patch_ratio, max_patch_ratio)))
    y0, x0 = _random_patch_bounds(h, w, ph, pw)
    out = img_01.clone()
    noise = torch.randn(3, ph, pw, dtype=out.dtype) * 0.15
    out[:, y0:y0 + ph, x0:x0 + pw] = torch.clamp(out[:, y0:y0 + ph, x0:x0 + pw] + noise, 0., 1.)
    mask = torch.zeros((1, h, w), dtype=torch.float32)
    mask[:, y0:y0 + ph, x0:x0 + pw] = 1.0
    return out, mask


class ImageFolderWithPseudoMask(torch.utils.data.Dataset):
    """
    MVTec-style train folder (ImageFolder layout): load images with the same geometry as
    get_data_transforms (Resize -> CenterCrop), optional pseudo-anomaly, then Normalize.
    Returns (img, label, gt_mask) for training; gt_mask is [1,H,W] float in {0,1}.
    """

    def __init__(self, root, class_label_idx, image_size, crop_size,
                 pseudo_prob=0.5, min_patch_ratio=0.02, max_patch_ratio=0.15,
        ...
        self.samples = [(path, class_label_idx) for path, _ in base.samples]
    ...
    def __getitem__(self, index):
        ...
        gt_mask = torch.zeros((1, t.shape[1], t.shape[2]), dtype=torch.float32)
        if random.random() < self.pseudo_prob:
            if random.random() < 0.5:
                t, gt_mask = _apply_cutpaste_tensor(t, self.min_patch_ratio, self.max_patch_ratio)
            else:
                t, gt_mask = _apply_gaussian_noise_patch_tensor(t, self.min_patch_ratio, self.max_patch_ratio)
        t = transforms.functional.normalize(t, self.mean_train, self.std_train)
        return t, label, gt_mask


class RealIADDatasetTrainWithPseudoMask(torch.utils.data.Dataset):
    """
    Real-IAD ``train`` split: same Resize/CenterCrop + pseudo anomaly as ImageFolderWithPseudoMask.
    Returns (img, label, gt_mask). Use RealIADDataset(phase='test') for evaluation.
    """

    def __init__(self, root, category, image_size, crop_size,
                 pseudo_prob=0.5, min_patch_ratio=0.02, max_patch_ratio=0.15,
                 mean_train=None, std_train=None):
        mean_train = [0.485, 0.456, 0.406] if mean_train is None else mean_train
        std_train = [0.229, 0.224, 0.225] if std_train is None else std_train
